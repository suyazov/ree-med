...
function trimed_render_responsive_picture($url, $args = array()) {
    ...
    $mobile_url = trim((string)$args['mobile_url']);
    if ($mobile_url === '') {
        $mobile_url = preg_replace('/\.(png|jpe?g|webp)(\?.*)?$/i', '-mobile.$1$2', $base_url);
    }

    $mobile_path = str_replace(
    ...
    $alt_attr = ' alt="' . esc_attr($args['alt']) . '"';
    $width_attr = is_numeric($args['width']) && (int)$args['width'] > 0 ? ' width="' . esc_attr((int)$args['width']) . '"' : '';
    $height_attr = is_numeric($args['height']) && (int)$args['height'] > 0 ? ' height="' . esc_attr((int)$args['height']) . '"' : '';
    $style_attr = $args['style'] !== '' ? ' style="' . esc_attr($args['style']) . '"' : '';

    $picture = sprintf(
        '<picture><source media="%s" srcset="%s"><img src="%s"%s%s%s%s%s></picture>',
        esc_attr($args['media']),
        esc_url($mobile_url),
        esc_url($base_url),
        $class_attr,
...
function trimed_render_slider_dots($count, $args = array()) {
    $count = (int)$count;
    if ($count <= 1) {
        return '';
    }
    ...
    $args = wp_parse_args($args, array(
        ...
        'data_attribute'   => 'data-slide',
        'echo'             => true,
        'icon'             => '',
    ));

    $active_index = max(0, (int)$args['active_index']);
    $count = max(0, $count);
    $tag = in_array($args['tag'], array('button', 'span', 'div'), true) ? $args['tag'] : 'button';

    $wrapper_class = trimed_sanitize_class_list($args['wrapper_class']);
    $base_class = trimed_sanitize_class_list($args['base_class']);
    $active_class = sanitize_html_class($args['active_class']);
    ...
    for ($i = 0; $i < $count; $i++) {
        $class_name = $base_class . ($i === $active_index ? ' ' . $active_class : '');
        $label = $args['aria_prefix'] . ' ' . ($i + 1);
        $attrs = '';
        if ($tag === 'button') {
            $attrs .= ' type="button" aria-label="' . esc_attr($label) . '"';
        }
        if (!empty($args['data_attribute'])) {
            $attrs .= ' ' . esc_attr($args['data_attribute']) . '="' . esc_attr($i) . '"';
        }
